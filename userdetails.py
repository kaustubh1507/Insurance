...
with open('decision_tree_pipeline_pre_1.2.2', 'rb') as f1:
...
def create_customer_profile():
    ...
    customer_lifespan = st.slider('Customer Lifespan in years', min_value=0, max_value=50, value=5)
...
def purchase():
    ...
    claim_probability = prediction_proba[0][1] * 100
    ...
    if fraud_probability > 0.5:
        st.write(f"There is a {(fraud_probability*100):.2f}% probability the person is fraud")
        ...
        fraud_probability = (1-fraud_probability)*100
        st.write(f"There is a {fraud_probability:.2f}% probability the person is genuine")
    ...
    if st.button('Issue Policy'):
        ...
        st.session_state.step = 1  # Go back to the previous step (Recommended Premium page)
...
def policy_issued():
    ...
    clv = abs(gbpipeline.predict(user_data_df)[0])

    st.write(f"Customer Lifetime Value (CLV): {clv:.2f}")

    acquired_value = st.session_state.acceptable_premium
    accured_value=st.session_state.acceptable_premium * st.session_state.customer_profile['customer_lifespan'] - st.session_state.acceptable_premium

    unrealized_value = clv - (acquired_value + accured_value)
    unrealized_status = "Unrealized Profit" if unrealized_value > 0 else "Unrealized Loss"
    st.write(f'Unrealized value: {abs(unrealized_value):.2f}')  # Display the absolute value for clarity
